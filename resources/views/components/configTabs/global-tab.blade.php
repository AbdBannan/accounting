<form action="{{route("config.saveConfig")}}" method="post">
    ...
    <div class="form-group row">
        ...
        <select value="{{$config["language"]}}" id="language" name="language" class="form-control col-3">
            <option value="arabic" @if($config["language"] == "arabic") selected @endif>{{__("global.arabic",[],session("lang"))}}</option>
            <option value="english" @if($config["language"] == "english") selected @endif>{{__("global.english",[],session("lang"))}}</option>
        </select>
    </div>
    ...
    <div class="form-group row">
        ...
        <select id="default_pound" name="default_pound" class="form-control col-3">
            @foreach(App\Models\Pound::all() as $pound)
                <option value="{{$pound->name}}" class="dropdown-item" @if($config["default_pound"] == $pound->name) selected @endif>{{__("global.$pound->name",[],session("lang"))}}</option>
            @endforeach
