    ...
    @section('content')
        <div class="container">
            ...
            @if($actions != null)
                ...
                <div class="p-2">
                    <a href="{{route("discover.chooseListCashDiscover")}}" class="btn btn-outline-primary">{{__("global.discovers")}}</a>
            ...
            @else
                <div id="accordion" class="bg-gradient-light shadow p-md-5 p-sm-2" style="width: 50%;margin: auto;">
                    <div id="buttons">
                        <button data-target="#cashDiscoverUntilNowCollapse" class="btn btn-primary btn-block">{{__("global.discover_until_now")}}</button>
                        <button data-target="#cashDiscoverAfterLastCheckedPointCollapse" class="btn btn-primary btn-block">{{__("global.discover_until_last_checked_point")}}</button>
                        <button data-target="#cashDiscoverBetweenTowDatesCollapse" class="btn btn-primary btn-block">{{__("global.discover_between_tow_dates")}}</button>
                    </div>
                    ...
                    <div id="cashDiscoverBetweenTowDatesCollapse" class="collapse" >
                        <a id="back"><i @if(app()->getLocale() == "en") class="fas fa-arrow-left" @else class="fas fa-arrow-right" @endif title="{{__("global.back")}}"></i></a>
                        <form id="form_discover" style="margin: auto" action="{{route("discover.cashDiscoverBetweenTowDates")}}" autocomplete="off">
                            <div class="position-relative form-group text-center">
                                <label style="font-size: x-large" for="account" >{{__("global.account")}}</label>
...
{{--                                <input tabindex="7" id="to" name="to" class="col-md-8 col-sm-12 form-control" type="date">--}}
    ...
    @section("script")
        <script>
            ...
            $("#date_container_from").buildDateInput({classes: "form-control",id:"from",tabindex:6,focusElemAfterFinish:"to",form:"form_discover",initialDate:"{{\Carbon\Carbon::now()->format("Y-m-d")}}"});
            $("#date_container_to").buildDateInput({classes: "form-control",id:"to",focusElemAfterFinish:"btn_cash_between_tow_dates",form:"form_discover",initialDate:"{{\Carbon\Carbon::now()->format("Y-m-d")}}"});
