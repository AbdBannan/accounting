<x-masterLayout.master>
    @if(count($invoiceLines)==0)
        @section("title")
            {{ __("global.search_in_invoices") }}
        @endsection
    @else
        @section("title")
            {{ __("global.".$invoiceLines[0]->invoice_type) }}
        @endsection
    @endif
                ...
                <x-invoices.invoice-body :invoiceType="$invoiceLines[0]->invoice_type">
                    @section("edit_delete")
                          ...
                          <div class="row">
                              ...
                              <a id="btn_delete" title="{{__("global.delete")}}" class="col-4" data-toggle="modal" data-target="#deleteConfirmModal" @if(auth()->user()->getConfig("use_recyclebin") == "true") data-route="{{route("invoice.softDeleteInvoice",$invoiceLines[0]->invoice_id)}}" @else data-route="{{route("invoice.deleteInvoice",$invoiceLines[0]->invoice_id)}}" @endif>
