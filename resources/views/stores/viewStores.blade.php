<x-masterLayout.master>
    ...
    @section("recycle_bin")
        <a class="dropdown-item" href="{{route("store.viewRecyclebin")}}">
            ...
            {{__("global.recycle_bin",["attribute"=>__("global.stores",[],session("lang"))],session("lang"))}}
        </a>
    @endsection
                ...
                <div>
                    <div>
                        <div class="form-group">
                            <a id="btn-add" title="{{__("global.add",[],session("lang"))}}" class="btn btn-sm btn-info" href="#" data-toggle="modal" data-target="#addModal" data-route="{{route("store.storeStore")}}">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($stores as $store)
                                            ...
                                            <tr>
                                                ...
                                                <td class="row m-0">
                                                    <a id="btn-update" title="{{__("global.update",[],session("lang"))}}" class="dropdown-item col-7 m-0 p-0" href="#" data-toggle="modal" data-target="#updateModal" data-fields="{{$store}}" data-route="{{route("store.updateStore",$store->id)}}">
                                                        <input class="grid-button grid-edit-button" type="button" title="Update">
                                                    </a>
                                                    <a id="btn-delete" title="{{__("global.delete",[],session("lang"))}}" class="dropdown-item col-5 m-0 p-0" href="#" data-toggle="modal" data-target="#deleteConfirmModal" data-route="{{route("store.softDeleteStore",$store->id)}}">
